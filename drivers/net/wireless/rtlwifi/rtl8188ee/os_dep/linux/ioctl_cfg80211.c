...
void rtw_cfg80211_init_rfkill(_adapter *adapter)
{
	wiphy_rfkill_set_hw_state(adapter->wiphy, 0);
	wiphy_rfkill_start_polling(adapter->wiphy);
}

void rtw_cfg80211_deinit_rfkill(_adapter *adapter)
{
	wiphy_rfkill_stop_polling(adapter->wiphy);
}
...
static void cfg80211_rtw_rfkill_poll(struct wiphy *wiphy)
{
	...
	if (adapter_to_dvobj(padapter)->processing_dev_remove == _TRUE) {
		/*DBG_871X("cfg80211_rtw_rfkill_poll: device is removed!\n");*/
		return;
	}

	blocked = rtw_hal_rfkill_poll(padapter, &valid);
	/*DBG_871X("cfg80211_rtw_rfkill_poll: valid=%d, blocked=%d\n",
			valid, blocked);*/
