...
bool rtw_hal_rfkill_poll(_adapter *adapter, u8 *valid)
{
	...
	if (adapter->HalFunc.hal_radio_onoff_check) {
		ret = adapter->HalFunc.hal_radio_onoff_check(adapter, valid);
		//printk("----> rtw_hal_rfkill_poll: valid=%d, ret=%d\n", *valid, ret);
		return ret;
	} else {
		*valid = 0;
		printk("--> rtw_hal_rfkill_poll: valid=%d, ret=0\n", *valid);
		return _FALSE;
	}
}
#endif
...
{	
	...
	#ifdef CONFIG_RFKILL_POLL
	if (NULL == padapter->HalFunc.hal_radio_onoff_check) {
