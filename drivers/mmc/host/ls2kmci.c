...
{
	return ls2k_gpio_read_input_pin(gpio);
}
#endif
...
static int ls2k_mci_probe(struct platform_device *pdev)
{
	...
	if (r == NULL) {
		dev_err(&pdev->dev, "no DMA resource defined\n");
		ret = -ENODEV;
		goto probe_free_host;
	}
	...
	ls2k_apbdma_cfg &=  ~(LS2K_APBDMA_MASK << LS2K_SDIO_DMA_SHIFT);
	ls2k_apbdma_sel = (r->start - LS2K_DMA0_REG) >> 4;
	ls2k_apbdma_cfg |= (ls2k_apbdma_sel & LS2K_APBDMA_MASK) << LS2K_SDIO_DMA_SHIFT;
