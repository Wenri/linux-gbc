...
#ifdef CONFIG_LOONGSON_WESTONE_SE
...
void loongson_se_dispatch(void)
{
	int irq, irqs;
	...
	while((irq = ffs(irqs)) != 0){
		irq = irq - 1;
		irqs &= ~(1 << irq);
		do_IRQ(LOONGSON_WESTONE_SE_BASE_IRQ + irq - 16);
	}
}
...
static inline void mask_loongson_irq(struct irq_data *d)
{
	...
	if ((d->irq != LOONGSON_UART_IRQ) && (d->irq < LOONGSON_WESTONE_SE_BASE_IRQ))
		return;
	/* Workaround: UART IRQ may deliver to any core */
	if (d->irq == LOONGSON_UART_IRQ)
		ls64_conf_write32(1 << 10, LS_IRC_ENCLR);
	else
		ls64_conf_write32(1 << (16 + d->irq - LOONGSON_WESTONE_SE_BASE_IRQ), LS_IRC_ENCLR);
#else
	if(!desc->action)
		return;
...
static inline void unmask_loongson_irq(struct irq_data *d)
{
	...
	if (d->irq == LOONGSON_UART_IRQ)
		...
		ls64_conf_write32(1 << (16 + d->irq - LOONGSON_WESTONE_SE_BASE_IRQ), LS_IRC_ENSET);
#else
	if (d->irq == LOONGSON_UART_IRQ)
		ls64_conf_write32(1 << 10, LS_IRC_ENSET);
...
void __init mach_init_irq(void)
{
	...
	for (i = 0; i < LOONGSON_WESTONE_SE_IRQ_NUM; i++) {
		unsigned data = 0;
		irq_set_chip_and_handler(LOONGSON_WESTONE_SE_BASE_IRQ + i,
			&loongson_irq_chip, handle_level_irq);
